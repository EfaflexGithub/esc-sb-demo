...
/// Signature for the [AdaptiveTextField.buildCounter] callback.
typedef InputCounterWidgetBuilder = Widget? Function(
  /// The build context for the AdaptiveTextField.
  BuildContext context, {
  /// The length of the string currently in the input.
  required int currentLength,

  /// The maximum string length that can be entered into the AdaptiveTextField.
  required int? maxLength,

  /// Whether or not the AdaptiveTextField is currently focused. Mainly provided for
...
///
/// To integrate the [AdaptiveTextField] into a [Form] with other [FormField] widgets,
...
///
/// {@tool dartpad}
/// This example shows how to create a [AdaptiveTextField] that will obscure input. The
...
/// ## Reading values
///
/// A common way to read a value from a AdaptiveTextField is to use the [onSubmitted]
/// callback. This callback is applied to the text field's current value when
/// the user finishes editing.
///
/// {@tool dartpad}
/// This sample shows how to get a value from a AdaptiveTextField via the [onSubmitted]
/// callback.
///
...
/// callback.
///
/// Keep in mind you can also always read the current string from a AdaptiveTextField's
...
///    [AdaptiveTextField]. The [EditableText] widget is rarely used directly unless
...
class AdaptiveTextField extends StatefulWidget {
  ...
  /// [AdaptiveTextField.noMaxLength] then only the current length is displayed.
  ///
  ...
  const AdaptiveTextField({
    ...
    this.magnifierConfiguration,
        ...
        assert(maxLength == null ||
            maxLength == AdaptiveTextField.noMaxLength ||
            maxLength > 0),
  ...
  /// The minimum width of the [AdaptiveTextField].
  final double minWidth;

  /// The maximum width of the [AdaptiveTextField].
  ...
  /// to [AdaptiveTextField.noMaxLength] then only the current character count is displayed.
  ///
  /// After [maxLength] characters have been input, additional input
  /// is ignored, unless [maxLengthEnforcement] is set to
  /// [MaxLengthEnforcement.none].
  ...
  /// which is evaluated after the supplied [inputFormatters], if any.
  ///
  /// This value must be either null, [AdaptiveTextField.noMaxLength], or greater than 0.
  /// If null (the default) then there is no limit to the number of characters
  /// that can be entered. If set to [AdaptiveTextField.noMaxLength], then no limit will
  ...
  /// "spinner" buttons that increment and decrement a value in the [AdaptiveTextField]
  ...
  ///
  /// The [mouseCursor] is the only property of [AdaptiveTextField] that controls the
  ...
  /// Default builder for [AdaptiveTextField]'s spell check suggestions toolbar.
  ///
  ...
  ///    parameter for which this is the default value for [AdaptiveTextField].
  ...
  static SpellCheckConfiguration inferAndroidSpellCheckConfiguration(
    ...
    return configuration.copyWith(
      misspelledTextStyle: configuration.misspelledTextStyle ??
          AdaptiveTextField.materialMisspelledTextStyle,
      spellCheckSuggestionsToolbarBuilder:
          configuration.spellCheckSuggestionsToolbarBuilder ??
              AdaptiveTextField.defaultSpellCheckSuggestionsToolbarBuilder,
    );
  }

  @override
  State<AdaptiveTextField> createState() => _TextFieldState();
...
class _TextFieldState extends State<AdaptiveTextField>
  ...
  Widget build(BuildContext context) {
    ...
    switch (defaultTargetPlatform) {
      ...
      case TargetPlatform.windows:
        spellCheckConfiguration =
            AdaptiveTextField.inferAndroidSpellCheckConfiguration(
          widget.spellCheckConfiguration,
        );
    }
    ...
    switch (theme.platform) {
      ...
      case TargetPlatform.macOS:
        ...
        handleDidGainAccessibilityFocus = () {
          // Automatically activate the AdaptiveTextField when it receives accessibility focus.
          if (!_effectiveFocusNode.hasFocus &&
    ...
    Widget child = RepaintBoundary(
      child: UnmanagedRestorationScope(
        ...
        child: EditableText(
          ...
          mouseCursor:
              MouseCursor.defer, // AdaptiveTextField will handle the cursor
...
// BEGIN GENERATED TOKEN PROPERTIES - AdaptiveTextField
...
// END GENERATED TOKEN PROPERTIES - AdaptiveTextField
